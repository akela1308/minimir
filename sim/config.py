...
class Config:
    ...
    # --- этап B: мир, который меняет правила (когда выгодно учиться) ---
    # Два типа еды, различимые сенсором. В каждый момент один тип питателен,
    # другой ядовит. Какой из них какой, переключается в НЕПРЕДСКАЗУЕМЫЕ
    # моменты: интервал между сменами равномерен на [T(1-j), T(1+j)].
    # Геном такого записать не может (моменты случайны), выучить при жизни
    # можно. Это и есть условие, при котором, по Kozielska и Weissing (2024),
    # отбор должен включать прижизненное обучение, а не выключать его.
    food_types: int = 1             # 1 = старый мир (один тип еды), 2 = новый
    type_patch_freq: float = 0.12   # пятна типа мельче пятен еды: оба типа
                                    # обычно есть в окрестности агента
    switch_mean: int = 0            # T: средний интервал между сменами, 0 = никогда
    switch_jitter: float = 0.5      # j: разброс интервала, доля от T
    switch_from_tick: int = 5000    # прогрев: правило начинает меняться только
                                    # на выжившей популяции (как social_from_tick)
    toxic_factor: float = -1.0      # энергия ядовитой еды относительно питательной
                                    # (-1 = теряешь столько же, сколько получил бы)
    # «Вкус»: одна пластичная связь цвет -> «есть». Логит действия «есть»
    # получает прибавку taste * color, если под ногами есть еда. Знак вкуса
    # и есть врождённое или выученное знание «какой цвет съедобен».
    # Это единственное, что учится при жизни в протоколе B: минимальная
    # пластичность, чтобы вопрос был про КОГДА обучение выгодно, а не про
    # то, удержит ли общее правило Хебба всё поведение (не удерживает,
    # проверено: при скорости, достаточной для переучивания за несколько
    # укусов, общее правило разваливает политику за сотню тиков).
    taste: bool = False
    taste_init: float = 3.0         # предок: ест цвет +1, избегает -1 (3 = не абсолютно:
                                    # при 5 смена правила убивает всех до единого)
    taste_clip: float = 6.0
    taste_lr_init: float = 0.1      # стартовая скорость обучения вкуса (в 30 раз
                                    # ниже той, при которой обучение спасает популяцию)
    taste_lr_sigma: float = 0.3     # мутация скорости: множитель exp(N(0, sigma))
    taste_lr_max: float = 4.0
    taste_deadzone: float = 0.5     # изменения энергии меньше порога правило не видит
    taste_learning: bool = True     # False = ген скорости есть и мутирует, но
                                    # правило не применяется (абляция обучения)
    ...
    hebb_deadzone: float = 0.0      # мёртвая зона модулятора: изменения энергии
                                    # меньше этого порога (обычный обмен веществ,
                                    # шаг, поворот) правило не видит. 0 = как в
                                    # пилотах C: модулятор работает каждый тик и
                                    # медленно размывает веса даже без событий
    ...
    # Этап B: скорость обучения на КАЖДОМ синапсе выходного слоя своя и
    # закодирована в геноме (как в Polyworld). Одна скорость на весь мозг
    # не работает: чтобы переучить один рефлекс за несколько укусов, нужна
    # скорость ~0.5, а с такой скоростью на всех синапсах поведение
    # разваливается за сотню тиков (проверено в пилоте). Пусть отбор сам
    # решает, ГДЕ обучение выгодно.
    hebb_per_synapse: bool = False
    hebb_rate_log: bool = False     # мутация скорости: множитель exp(N(0, sigma)),
                                    # а не прибавка N(0, sigma). Только так скорость
                                    # может вырасти на порядки за десятки поколений
    hebb_rate_max: float = 4.0
    # Барьер Вейсмана: прижизненные изменения весов НЕ наследуются. Ребёнок
    # получает мутированный геном родителя, а не его текущие (выученные) веса.
    # До сентября 2026 барьера не было: в пилотах C.1-C.4 дети наследовали
    # выученные веса (ламарковское наследование). Флаг оставлен только для
    # воспроизведения тех пилотов.
    lamarck: bool = False
    ...
    def __post_init__(self):
        if self.food_types not in (1, 2):
            raise ValueError("food_types: только 1 или 2")
        if self.food_types == 2 and self.signs:
            # входы 23-24 в мире с двумя типами еды заняты цветом (см. SENSOR_NAMES)
            raise ValueError("food_types=2 несовместимо со знаковым слоем (signs=True)")
        if self.switch_mean and self.food_types != 2:
            raise ValueError("switch_mean имеет смысл только при food_types=2")
        if self.food_types == 2 and self.n_hidden < 2:
            raise ValueError("food_types=2: нужен хотя бы один нейрон под рефлекс")
...
# В мире с двумя типами еды (food_types=2) знаковый слой выключен, и слоты
# 23-24 несут цвет еды: под собой и впереди (-1 / +1). Форма генома при этом
# не меняется, поэтому предки и браузерный движок совместимы по размеру.
I_COLOR_HERE, I_COLOR_AHEAD = 23, 24
SENSOR_NAMES_FOOD2 = SENSOR_NAMES[:23] + ["color_here", "color_ahead"]
